# Getting Results for Ross et. al
# This code is different b/c there is no ARL that needs to be calculated
...
# Most Important
# f_in_control <- Find_CL_CVM_Pval_DWindow
f_out_control <- Ross_OOC_ARL

# TEMPORARY
method <- "Ross"
lambdas = c(1) # double window change
...
ooc_type <- "mean"
shifts <- c(seq(-1, -.2, by=.2), seq(0.2, 1, by=.2))
...
# SD SHIFTS ---------------------------------------------------------------
ooc_type <- "sd"
shifts <- c(seq(.2, .8, by=.2), seq(1.2, 2, by=.2))
