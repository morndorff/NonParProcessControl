# Working with a mean variance standardized Anderson Darling Statistic
...
# Most Important
f_in_control <- Find_CL_AD_Window
f_out_control <- Find_CL_AD_Window_OOC
...
# W = 40
# f_mem = memoise(Fast_Bootstrap_CVM_Weighted)

# for(j in 1:length(lambdas)){
#   for(i in 1:W){
#     current.weight = rep((1-lambdas[j])^(i - 1:i), each=m)
#     current.weight = current.weight/sum(current.weight)
#     # out[[i]] = f_mem(ic_sample_size=M, m=m*i, w= current.weight)
#     # print(i)
#   }
# }
# proc.time()-ti
...
# ti = proc.time()
# for(j in 1:length(lambdas)){
#     EWMA_Find_CL_CVM_Weight(ic_data=rnorm(M), lambda=lambdas[j], control_limit = 5, W=W, diag=TRUE)
#     # current.weight = rep((1-lambdas[j])^(i - 1:W), each=m)
#     # current.weight = current.weight/sum(current.weight)
#     # out[[i]] = f_mem(ic_sample_size=M, m=m*i, w= current.weight)
#     print(j)
# }
...
# CL_Finding_With_UB(allow_param=.01, cl_seq=c(.2, .8), f_in_control=f_in_control, sd_tol_init=10, pval_approach=FALSE)
...
if(method=="weighted_bs"){
  if(exists("W")){
    save(data.results, tstat, method, ARL, m, M, W, file=save_name)
  }
}else{
