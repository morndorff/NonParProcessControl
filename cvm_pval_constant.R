# Replication of Partha Paper with CVM
...
lambdas = c(.03, .06, .09, .12, .15) # constant percent pruned
num_lambdas <- length(lambdas) 
method = "constant" # Use constant pruning method

# Accuracy of estimate of UCLs
sd_tol_init = 3
ucl <- c(.02, .02, .02, .02, .02)
names(ucl) <- as.character(lambdas)
lcl <- c(.005, .004, 0.004, 0.004, 0.004)
...
for (j in seq_along(lambdas)) {
  for (i in seq_along(shifts)) {
    ...
    while (sd_arl > sd_tol[i]) {
      #method=method is the change here!
...
README_OOC <- "Mean SHIFT RESULTS FOR CVM PVAL, CONSTANT METHOD, ARL 200"
save_name <-nameresults("cvm_pval_constant_ewma_200_mean_shift")
...
README_OOC <- "SD SHIFT RESULTS FOR CVM PVAL, CONSTANT METHOD, ARL 200"
save_name <-nameresults("cvm_pval_constant_ewma_200_sd_shift")
save(CL_200_Final, lambdas, README_OOC, ARL, m, M, sd_tol_init, 
     shifts, results_matrix, results_matrix_sd, N2_min, file=save_name)
